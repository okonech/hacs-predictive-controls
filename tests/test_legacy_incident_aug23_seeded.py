"""Legacy qualification relocated from the Aug23 0556Z incident, not acceptance.

Source: /tmp/black-box-migration-baseline.json, original
tests/incidents/test_inc_2026_08_23_0556z_asserted_stay_never_releases_for_count_conflict.py.
Original report text is unavailable here; the retained fixture reconstructs an
asserted target falsely releasing during count conflict. Synthetic outside paths
and the scalar-seeded posterior are not a complete captured production history.
2026-09-14: authentic count/filter/policy component ownership replaces the obsolete
selected-engine count-degradation premise; all numbers and timestamps remain.
Event-only public retention and sampled health now live in the incident file;
this preserves the non-equivalent seeded boundary and any real legacy failure.
"""
...
def test_legacy_aug23_0556z_asserted_stay_seeded_count_conflict() -> None:
    ...
    engine.evaluate_count_conflicts(conflict_started_at)
    ...
    engine.evaluate_count_conflicts(conflict_deadline)
    evaluations = [
        engine.advance(conflict_deadline),
    ...
    # Count0 is categorical even while the legacy count-degraded assertion holds.
    zero_at = observed_release_at + timedelta(microseconds=1)
    for current in (engine, restored):
        cleared = current.observe_count(CountInput(
            event_id="legacy-count-zero", value=0, available=True, event_at=zero_at,
        ))
        assert not any(policy.active for policy in cleared.snapshot.policy_states)
        assert cleared.snapshot.count_conflicts == ()
        assert cleared.snapshot.anonymous_supports == ()
        assert cleared.snapshot.traversal_tokens == ()
    assert restored.snapshot == engine.snapshot
